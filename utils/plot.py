...
def get_plot_indices(image: np.ndarray, n_slices: int = 16) -> Tuple[int, int]:
    ...
    print("max slice sum:", np.max(sum_line))
    index_start, index_end = get_biggest_island_indices(sum_line > 0) #sum_line > 300
    print("number of viable slices over 300 threshold: ", np.sum(sum_line > 300))
    print("index_start:", index_start)
    print("index_end:", index_end)
    flt_inter = (index_end - index_start) // n_slices
    print("the float interval is:", flt_inter)
...
def make_montage(image: np.ndarray, n_slices: int = 16) -> np.ndarray:
    ...
    return montage
def make_full_montage(image: np.ndarray, n_slices: int = 16) -> np.ndarray:
    """Plot a montage of ALL slices in grayscale."""
    ...
    print(f"Building montage with {z} slices...")

    # 🔥 Automatically choose grid size
    n_cols = int(np.ceil(np.sqrt(z)))
    n_rows = int(np.ceil(z / n_cols))

    # Create empty montage canvas
    montage = np.zeros((n_rows * x, n_cols * y, 3))
...
def plot_montage_grey(
    ...
    plt.clf()
    plt.close()
    print("donzo with montaging grey")
    return montage
def plot_montage_color(
    image: np.ndarray,
    path: str,
    index_start: int,
    index_skip: int = 1, #original =  1
    n_slices: int = 16,
):
    ...
    """
    print("the index_skip is:", index_skip)
    #index_skip = 3 ##comment this out later
    # plot the montage
    ...
    '''
    montage = make_full_montage(
        image[:, :, :, :], n_slices=n_slices
    )
    '''
    z = '''index_start:index_end:index_skip'''
    plt.figure()
    plt.imshow(montage, cmap="gray")
    plt.axis("off")
    plt.savefig(path, transparent=True, bbox_inches="tight", pad_inches=-0.05, dpi=300)
    ...
    plt.close()
    return montage
def plot_all_montages(montages: List[np.ndarray]):
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))  # 1 row, 3 columns

    # Gas
    axes[0].imshow(montages[0], cmap="gray")
    axes[0].set_title("Gas")
    axes[0].axis("off")

    # Membrane
    axes[1].imshow(montages[1] , cmap="gray")
    axes[1].set_title("Membrane")
    axes[1].axis("off")

    # Proton
    axes[2].imshow(montages[2], cmap="gray")
    axes[2].set_title("RBC")
    axes[2].axis("off")
