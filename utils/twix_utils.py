...
def get_gx_data(twix_obj: mapvbvd._attrdict.AttrDict) -> Dict[str, Any]:
    ...
    print("\n>>> Begin read of gas-exchange data from TWIX file.")
    ...
    print(f"\nGas flip angle:          {flip_angle_gas:.2f}"     )
    print(f"Diss. flip angle:        {flip_angle_dissolved:.2f}" )
    print(f"raw_fids no. of rows:    {raw_fids.shape[0]:5d}"     )
    print(f"raw_fids no. of columns: {raw_fids.shape[1]:5d}\n"   )
    ...
    if flip_angle_dissolved == 12:
    ...
    elif flip_angle_dissolved == 10.8:
         if raw_fids.shape[0] == 4320:
            logging.info("Reading in 'E-Cig' dixon data on Siemens Prisma.")

            # temporary patch to handle E-Cig protocol data
            # correct data handling should be based on reading protocol parameters, not hard-coded as done here
            #
            # extract imaging data from raw_fids array
            num_readpts      = 64   # these hard-coded values are a real kludge
            num_spectra_pre  = 100
            num_spectra_post = 20
            raw_fids         = raw_fids[num_spectra_pre:-num_spectra_post][0::1,:num_readpts]

            # get gas and dissolved views
            data_gas         = raw_fids[0::2, :]
            data_dis         = raw_fids[1::2, :]
            n_frames         = data_dis.shape[0]
            n_skip_start     = 0
            ...
            raise ValueError("Cannot get data from 'E-Cig' dixon twix object.")
        ...
        if raw_fids.shape[0] == 2030:
            ...
            # temporary patch to handle old data sets that acquired out to 2*kmax (128 pts) vs. kmax (64 pts)
            # correct data handling should be based on reading protocol parameters, not hard-coded as done here
            num_readpts = 64
            if raw_fids.shape[1] == 128:
              # discard data from kmax to 2*kmax
              raw_fids  = raw_fids[:][0::1,:num_readpts]
              print(f"raw_fids no. of rows:    {raw_fids.shape[0]:5d}"     )
              print(f"raw_fids no. of columns: {raw_fids.shape[1]:5d}\n"   )
    ...
    print("\n>>> End read of gas-exchange data from TWIX file.\n")
