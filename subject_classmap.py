...
class Subject(object):
    ...
    def reconstruction_gas(self):
        ...
        print()
        print("image_gas_highreso shape:", self.image_gas_highreso.shape)
        print("image_gas_highreso max:", np.max(np.abs(self.image_gas_highreso)))
        print("image_gas_highreso mean:", np.mean(np.abs(self.image_gas_highreso)))
        print()
    ...
    def generate_figures(self):
        ...
        grey_vent_montage = plot.plot_montage_grey(
            image=np.abs(self.image_gas_highreso),
            path="tmp/montage_vent.png",
            index_start=index_start,
            index_skip=index_skip,
        )
        grey_membrane_montage = plot.plot_montage_grey(
            image=np.abs(self.image_membrane),
            path="tmp/montage_membrane.png",
            index_start=index_start,
            index_skip=index_skip,
        )
        grey_rbc_montage = plot.plot_montage_grey(
            image=np.abs(self.image_rbc),
            path="tmp/montage_rbc.png",
            index_start=index_start,
            index_skip=index_skip,
        )
        plot.plot_all_montages([
            grey_vent_montage,
            grey_membrane_montage,
            grey_rbc_montage
        ])
        color_vent_montage = plot.plot_montage_color(
        ...
        color_rbc_montage = plot.plot_montage_color(
            image=plot.map_and_overlay_to_rgb(
        ...
        color_membrane_montage = plot.plot_montage_color(
            image=plot.map_and_overlay_to_rgb(
        ...
        plot.plot_all_montages([
            color_vent_montage,
            color_membrane_montage,
            color_rbc_montage
        ])
        qa_proton_montage = plot.plot_montage_color(
            image=plot.overlay_mask_on_image(proton_reg, self.mask.astype("uint8")),
            path="tmp/montage_proton_qa.png",
            index_start=index_start,
            index_skip=index_skip,
        )
        qa_vent_montage = plot.plot_montage_color(
        ...
        qa_dissolved_montage = plot.plot_montage_color(
            image=plot.overlay_mask_on_image(
    ...
    def generate_pdf(self):
        ...
        pdf_list = [
            ...
            for pdf in ["intro.pdf", "clinical.pdf", "grayscale.pdf", "qa.pdf"]
        ]
    ...
    def move_output_files(self):
        ...
        # move files
        subfolder = os.path.join(self.config.data_dir, "output")
